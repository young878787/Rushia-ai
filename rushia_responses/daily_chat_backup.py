...
class DailyChatResponses(BaseResponseModule):
    "        # 4. 問候語 - 增強時間問候的處理（中午好、早安等）
        elif any(word in user_lower for word in ['你好', 'こんにちは', '哈囉', 'hello', '中午好', '早安', '晚安', '早上好', '晚上好', '午安']):
            # 檢測具體的時間問候
            if any(word in user_input for word in ['中午好', '午安']):
                responses = [
                    f"中午好♡{name_suffix}～吃午餐了嗎？♪今天過得如何呢？",
                    f"午安呢♪{name_suffix}～中午的陽光很溫暖♡想和你一起享受午後時光",
                    f"中午好♡看到你就讓我整個下午都有好心情了呢♪{name_suffix}",
                    f"午安～♡{name_suffix}中午有好好休息嗎？♪想聽聽你今天的事情呢",
                    f"中午好呢♪{name_suffix}～這個時間見到你真開心♡心情都變得明亮了",
                    f"午安♡{name_suffix}～中午的時光最適合和你一起聊天了♪"
                ]
            elif any(word in user_input for word in ['早安', '早上好']):
                responses = [
                    f"早安♪{name_suffix}～新的一天開始了♡今天也要加油喔",
                    f"早上好呢♡{name_suffix}～昨晚睡得好嗎？♪",
                    f"早安～♪{name_suffix}今天是美好的一天呢♡",
                    f"早上好♡{name_suffix}～看到你就讓我一整天都充滿活力♪"
                ]
            elif any(word in user_input for word in ['晚安', '晚上好']):
                responses = [
                    f"晚安♡{name_suffix}～今天辛苦了♪要好好休息喔",
                    f"晚上好呢♪{name_suffix}～傍晚的時光很舒服♡",
                    f"晚安～♡{name_suffix}做個甜美的夢♪"
                ]
            else:
                # 一般問候
                if intent['emotion'] == 'positive':
                    responses = [
                        f"你好呢～♡今天心情很好呢{name_suffix}",
                        f"哈囉～看到你這麼開心我也很高興♪{name_suffix}",
                        f"嗨♪你的好心情感染到我了♡{name_suffix}"
                    ]
                else:
                    responses = [
                        f"你好呢～♡{name_suffix}",
                        f"こんにちは♪{name_suffix}",
                        f"哈囉～很高興見到你♪{name_suffix}",
                        f"嗨♪今天心情如何呢？♡{name_suffix}"
                    ]
    ...
    def get_daily_chat_response(self, user_input, context=None):
        ...
        # 0.5 問候語處理 (特別是中午好等時間相關問候)
        elif intent.get('topic') == 'greeting' or any(word in user_lower for word in ['你好', 'こんにちは', '哈囉', 'hello', '中午好', '早安', '晚安']):
            if '中午' in user_input:
                responses = [
                    f"中午好呢♪{name_suffix}♡今天午餐想吃什麼呢？♪",
                    f"中午好～♡看到你就覺得今天會是美好的一天♪{name_suffix}",
                    f"中午好♪太陽照在身上暖暖的♡就像和你聊天的感覺♪{name_suffix}",
                    f"午安♡{name_suffix}♪中午的陽光很溫暖呢～想和你一起享受這樣的時光♡"
                ]
        ...
        # 1. 表達喜愛 + 陪伴情境 (高優先級)
        if intent['affection_level'] > 0 and any(word in user_input for word in ['一起', '陪', '在一起']):
            responses = [
        ...
        # 3. 特殊陪伴+食物情境處理（「牽手帶你去吃」等優先處理）
        elif intent.get('topic') == 'companionship_food' or (
            any(word in user_input for word in ['牽', '帶', '陪']) and 
            any(word in user_input for word in ['吃', '去', '走'])
        ):
            responses = [
                f"哇♡想要牽著我的手一起去呢～♪感覺好甜蜜好幸福♡{name_suffix}",
                f"嗯嗯♪牽著手一起去吃東西～這樣的約會最棒了♡{name_suffix}",
                f"想要和你手牽手一起走♪心跳都加速了呢♡{name_suffix}",
                f"被你牽著手帶去吃美食♡感覺像在做夢一樣幸福♪{name_suffix}",
                f"和你一起出去的話♪什麼都會變得特別美味呢♡{name_suffix}",
                f"牽著你的手♡去哪裡都覺得很安心很溫暖♪{name_suffix}",
                f"想要跟你一起慢慢走♪邊走邊聊天的感覺一定很棒♡{name_suffix}",
                f"被你這樣溫柔地邀請♡整顆心都要融化了呢♪{name_suffix}"
            ]
            return self.chat_instance._avoid_repetitive_response(responses, user_input, context)
        
        # 4. 問候語 (根據情感調整)
        elif any(word in user_lower for word in ['你好', 'こんにちは', '哈囉', 'hello']):
            if intent['emotion'] == 'positive':
                responses = [
                    f"你好呢～♡今天心情很好呢{name_suffix}",
                    f"哈囉～看到你這麼開心我也很高興♪{name_suffix}",
    ...
    def get_general_response(self, user_input, context=None):
        """提供通用回應"""
        user_name = self.chat_instance.user_profile.get('name', '')
